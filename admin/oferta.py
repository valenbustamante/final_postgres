...
def add_academic_program(program, period, semesters, title, registration_fee, tuition_fee):
    ...
    try:
        cur.execute(
            """
            INSERT INTO oferta (programa, periodo, semestres, titulo, inscripcion, matricula)
            VALUES (%s, %s, %s, %s, %s, %s)
...
def add_subject(program_id, name, credits, semester, subject_code, description):
    ...
    try:
        cur.execute(
            """
            INSERT INTO asignaturas (id_programa, nombre, creditos, semestre, nombre_asignatura, descripcion)
            VALUES (%s, %s, %s, %s, %s, %s)
...
def add_requirement(program_id, requirement_name):
    conn = get_connection()
    cur = conn.cursor()
    try:
        cur.execute(
            """
            INSERT INTO requisitos (id_programa, nombre_doc)
            VALUES (%s, %s)
            """,
...
def delete_program(program_id):
    ...
    try:
        ...
        cur.execute("DELETE FROM requisitos WHERE id_programa = %s", (program_id,))
        cur.execute("DELETE FROM asignaturas WHERE id_programa = %s", (program_id,))
        cur.execute("DELETE FROM oferta WHERE id_programa = %s", (program_id,))
        conn.commit()
        return True
    except Exception as e:
        conn.rollback()
...
def delete_subject(subject_id):
    ...
    cur = conn.cursor()
    try:
        cur.execute("DELETE FROM asignaturas WHERE id_asignatura = %s", (subject_id,))
        conn.commit()
        return True
    except Exception as e:
        conn.rollback()
...
def delete_requirement(program_id, requirement_name):
    ...
    cur = conn.cursor()
    try:
        cur.execute("DELETE FROM requisitos WHERE id_programa = %s AND nombre_doc = %s", 
                   (program_id, requirement_name))
        conn.commit()
        return True
    except Exception as e:
...
def main():
    st.title("Configuración de Oferta Académica")
    ...
    selected_period = st.selectbox(
        ...
        format_func=lambda x: f"{x[:4]}-{x[4:]} ({'Primer' if x[4:] == '10' else 'Segundo'} semestre del año)"
    )
            ...
            if programa_seleccionado:
                with st.form("nuevo_requisito_form"):
                    ...
                    if submit_button and requisito:
                        if add_requirement(programa_seleccionado[0], requisito):
